...
$c = mysql_query("SELECT * FROM ledger_tabs WHERE glid = '".$b["company_id"]."' AND saasid = '".$_SESSION['saasid']."'");
...
$html.= '<div style="padding-top:90px"><span style="font-weight:bold;">Location:</span> NOT SET<br>
<span style="font-weight:bold;">Cost:</span> $'.$b["value"].'<br><span style="font-weight:bold;">Type:</span> '.$b["base_type"].'<br><br><span style="font-weight:bold;">Contact Info:</span><br> '.$dd["fld1"].'<br>'.$dd["addressset"].'<br>'.$dd["fld7"].' '.$dd["fld6"].', '.$dd["fld8"].'<br>'.$dd["vencon_phone"].'
</div>
...
$mpdf=new mPDF(); 
$mpdf->SetHTMLHeader('<div style="width:161px; height:64px; float:left; background-image:url(images/main_logo.gif)"></div><div style="float:left; text-align:right; font-size:11px">PO for: '.$dd["fld1"].'<br>Date Created: '.$b["issue_date"].'<br>Created By: ACS System</div>');
$mpdf->SetFooter('ACS PO | {PAGENO}');
$mpdf->WriteHTML($html);
$mpdf->Output();
