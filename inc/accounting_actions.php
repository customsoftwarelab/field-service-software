...
if($act == 'addexpense'){
    ...
    <div style="width:300px; height:40px; float:left">
    GL #:<br>
    <input style="float:left" class="makerrs text" name="glnum" id="glnum" type="text" value="">
    </div>
...
if($act == 'editxpense'){
    ...
    <div style="width:300px; height:40px; float:left">
    GL #:<br>
    <input style="float:left" class="makerrs text" name="glnum" id="glnum" type="text" value="'.$rt["setglnum"].'">
    </div>
...
///updatreexp&name
if($act == 'updatreexp'){
	mysql_query("UPDATE ledger_tabs SET fld1 = '".mysql_real_escape_string($_REQUEST['name'])."', setglnum = '".mysql_real_escape_string($_REQUEST['glnum'])."' WHERE glid = '".$_REQUEST['id']."' AND saasid = '".$_SESSION['saasid']."'");
...
if($act == 'subexpense'){
	...
	$glnum = mysql_real_escape_string($_REQUEST['setglnum']);
	
	mysql_query("INSERT INTO ledger_tabs SET fld1 = '$name', subacct = '$subacct', fld3 ='$setdate', fld4 ='$amount', type = 'expense', setglnum = '$glnum', saasid = '".$_SESSION['saasid']."', active = 'true'")or die(mysql_error());
}
...
$result = mysql_query($query)  or die(mysql_error());
	...
	if(mysql_num_rows($result) < 1 ){
		...
		echo '<div style="font-family: \'Quantico\', sans-serif; font-size:20px; color:#1053A3; font-style:italic; padding-left:15px; padding-top:15px;">No Expenses..</div>';
...
<div class="headtext2" style="width: 110px;">'.$setgl.'</div>
...
$yu = mysql_fetch_array(mysql_query("SELECT * FROM ledger_tabs WHERE glid = '".$h["glid"]."' AND saasid='".$_SESSION['saasid']."'"));
	
	
	$rt = mysql_query("SELECT * FROM transandpays WHERE gl_act = '".$h["glid"]."' AND status != '' AND saasid = '".$_SESSION['saasid']."'");
...
$rt2 = mysql_query("SELECT * FROM transandpays WHERE gl_act = '".$h["glid"]."' AND ispay = 'true' AND saasid = '".$_SESSION['saasid']."'");
...
$result2 = mysql_query($query2)  or die(mysql_error());
	...
	if(mysql_num_rows($result2) < 1 ){
		...
		echo '<div style="font-family: \'Quantico\', sans-serif; font-size:18px; color:#1053A3; font-style:italic; padding-left:15px; padding-top:15px;">No Expenses..</div>';
...
<div style="width:100px; float:left; height:25px;">$'.$pay.'</div>
	...
	echo '<div style="padding:5px; float:right; margin-right:60px;"><strong>Top Level Balance: $'.$owedAmount.'</strong></div>';
	
	
	
 $query3   = "SELECT COUNT(gl_act) AS numrows FROM transandpays WHERE gl_act = '".$h["glid"]."' AND saasid = '".$_SESSION['saasid']."'";
$result3  = mysql_query($query3) or die(mysql_error());
$row2     = mysql_fetch_array($result3, MYSQL_ASSOC);
$numrows2 = $row2['numrows'];
///echo $numrows;
// how many pages we have when using paging?
$maxPage = ceil($numrows2/$rowsPerPage2);
...
for($page = 1; $page <= $maxPage; $page++)
{
	...
	{
		$nav .= '<div class="pagnull" onclick="getLinesinner(\''.$h["glid"].'\',\''.$page.'\')">'.$page.'</div>';
	}		
}
...
{
	$page = $pageNum + 1;
	$next = '<div class="pagnull" onclick="getLinesinner(\''.$h["glid"].'\',\''.$page.'\')">Next</div></a>';
	
	//$last = " <a href=\"$self?page=$maxPage\">Last</a> ";
	
	$last = '<div class="pagnull" onclick="getLinesinner(\''.$h["glid"].'\',\''.$maxPage.'\')">Last</div>';
} 
else
{
...
';
echo '</div><!--<div id="delle" style="float:left; margin-left:25px">Delete</div>-->';
echo '</div>';

////END TOP LEVEL////////=======================================================================================================================================================================

echo '<div style="border-bottom:solid thin #999; height:30px; margin-bottom:25px;"></div>';
...
$result = mysql_query($query)  or die(mysql_error());
	...
	if(mysql_num_rows($result) < 1 ){
		...
		echo '<div style="font-family: \'Quantico\', sans-serif; font-size:25px; color:#FFCC00; font-style:italic; padding-left:15px; padding-top:15px;">No expenses in system..</div>';
	...
	echo '<div class="infoheadlines">
<div class="headtext2" style="width: 110px;">'.$glSet.'</div>
<div class="headtext2" style="width: 443px;">'.$h["fld1"].'</div>
...
$query  = "SELECT * FROM transandpays WHERE gl_act = '".$h["glid"]."' AND saasid = '".$_SESSION['saasid']."' ORDER BY gl_act $dirs LIMIT $offset, $rowsPerPage";
...
$result = mysql_query($query)  or die(mysql_error());
	...
	if(mysql_num_rows($result) < 1 ){
		...
		echo '<div style="font-family: \'Quantico\', sans-serif; font-size:18px; color:#FFCC00; font-style:italic; padding-left:15px; padding-top:15px;">No expenses in system..</div>';
...
<div style="width:100px; float:left; height:25px;">$'.$pay.'</div>
 ...
 $query   = "SELECT COUNT(gl_act) AS numrows FROM transandpays WHERE gl_act = '".$h["glid"]."' AND saasid = '".$_SESSION['saasid']."'";
$result  = mysql_query($query) or die(mysql_error());
$row     = mysql_fetch_array($result, MYSQL_ASSOC);
$numrows = $row['numrows'];
///echo $numrows;
...
$result = mysql_query($query)  or die(mysql_error());
	...
	if(mysql_num_rows($result) < 1 ){
		...
		echo '<div style="font-family: \'Quantico\', sans-serif; font-size:20px; color:#1053A3; font-style:italic; padding-left:15px; padding-top:15px;">No Results Found..</div>';
...
echo '<div style="width:603px; height:40px; margin-bottom:15px">
    <div style="width:300px; height:40px; float:left">
    Gl #:<br>
    <input style="float:left" class="makerrs text" name="glnum" id="glnum" type="text">
    </div> 
	<div style="width:300px; height:40px; float:left">
   <!-- Sub Account:<br>
...
$glnum = mysql_real_escape_string($_REQUEST['glnum']);
	

mysql_query("INSERT INTO ledger_tabs SET fld1 = '$bankname', subacct = 'none', fld3 ='$phone', addressset ='$address', fld5 ='$adrress2', fld6 = '$state', fld7='$city', fld8='$zip', fld9='$banknum', fld10='$routnum', fld11='$checknumstr', type = 'bank', setglnum = '$glnum', saasid = '".$_SESSION['saasid']."', active = 'true'")or die(mysql_error());
}
...
mysql_query("UPDATE ledger_tabs SET fld1 = '$bankname', subacct = 'none', fld3 ='$phone', addressset ='$address', fld5 ='$adrress2', fld6 = '$state', fld7='$city', fld8='$zip', fld9='$banknum', fld10='$routnum', fld11='$checknumstr', type = 'bank', setglnum = '$glnum' WHERE saasid = '".$_SESSION['saasid']."' AND glid = '".$_REQUEST['id']."'")or die(mysql_error());
}
...
<div style="width:603px; height:40px; margin-bottom:15px">
    <div style="width:300px; height:40px; float:left">
    GL #:<br>
    <input style="float:left" class="makerrs text" name="glnum" id="glnum" type="text" value="'.$rtb["setglnum"].'">
    </div> 
	<div style="width:300px; height:40px; float:left">
   <!-- Sub Account:<br>
...
$result = mysql_query($query)  or die(mysql_error());
	...
	if(mysql_num_rows($result) < 1 ){
		...
		echo '<div style="font-family: \'Quantico\', sans-serif; font-size:25px; color:#1053A3; font-style:italic; padding-left:15px; padding-top:15px;">No Expenses..</div>';
...
if($act == 'getledgdets'){
	...
	$rt = mysql_query("SELECT * FROM transandpays WHERE gl_act = '".$_REQUEST["glid"]."' AND status != '' AND saasid = '".$_SESSION['saasid']."' OR gl_refatt = '".$_REQUEST["glid"]."' AND saasid = '".$_SESSION['saasid']."' status != ''");
...
$rt2 = mysql_query("SELECT * FROM transandpays WHERE gl_act = '".$_REQUEST["glid"]."' AND ispay = 'true' AND saasid = '".$_SESSION['saasid']."' OR gl_refatt = '".$_REQUEST["glid"]."' AND saasid = '".$_SESSION['saasid']."' AND ispay = 'true'");
...
echo '<div id="contin">';	
echo '<div style="width:100%">
<strong>Details For:</strong><input name="gllnmm" id="gllnmm" type="text" value="'.$yu["fld1"].'"/> | <a href="javascript:changetheName(\''.$_REQUEST['glid'].'\')">Edit Name</a><br /><br />
...
$query  = "SELECT * FROM transandpays WHERE gl_act = '".$_REQUEST['glid']."' AND saasid = '".$_SESSION['saasid']."' OR gl_refatt = '".$_REQUEST["glid"]."' AND saasid = '".$_SESSION['saasid']."' ORDER BY gl_act $dirs LIMIT $offset, $rowsPerPage";
...
$i=0;
		...
		$rb = mysql_fetch_array(mysql_query("SELECT * FROM ledger_tabs WHERE glid = '".$s["paidby"]."' AND saasid = '".$_SESSION['saasid']."' OR gl_refatt = '".$_REQUEST["glid"]."' AND saasid = '".$_SESSION['saasid']."'"));
...
<div style="width:100px; float:left; height:25px;">$'.$pay.'</div>
 ...
 $query   = "SELECT COUNT(gl_act) AS numrows FROM transandpays WHERE gl_act = '".$_REQUEST['glid']."' AND saasid = '".$_SESSION['saasid']."' OR gl_refatt = '".$_REQUEST["glid"]."' AND saasid = '".$_SESSION['saasid']."'";
$result  = mysql_query($query) or die(mysql_error());
$row     = mysql_fetch_array($result, MYSQL_ASSOC);
$numrows = $row['numrows'];
///echo $numrows;
...
for($page = 1; $page <= $maxPage; $page++)
{
	...
	{
		$nav .= '<div class="pagnull" onclick="getLinesinner(\''.$_REQUEST["glid"].'\',\''.$page.'\')">'.$page.'</div>';
	}		
}
...
{
	$page = $pageNum + 1;
	$next = '<div class="pagnull" onclick="getLinesinner(\''.$_REQUEST["glid"].'\',\''.$page.'\')">Next</div></a>';
	
	//$last = " <a href=\"$self?page=$maxPage\">Last</a> ";
	
	$last = '<div class="pagnull" onclick="getLinesinner(\''.$_REQUEST["glid"].'\',\''.$maxPage.'\')">Last</div>';
} 
else
{
	$next = '&nbsp;'; // we're on the last page, don't print next link
...
echo '<div id="contin'.$_REQUEST["glid"].'">';
...
///first get name///
if($act == 'getname'){
	$yun = mysql_fetch_array(mysql_query("SELECT * FROM ledger_tabs WHERE glid = '".$_REQUEST['glid']."' AND saasid='".$_SESSION['saasid']."'"));
	echo $yun["fld1"];
}
...
$result = mysql_query($query)  or die(mysql_error());
	...
	if(mysql_num_rows($result) < 1 ){
		...
		echo '<div style="font-family: \'Quantico\', sans-serif; font-size:25px; color:#FFCC00; font-style:italic; padding-left:15px; padding-top:15px;">No Invoice Payments</div>';
...
<div style="width:120px; height:40px; float:left">
   Vendor:
    ...
    <option value="none">Select Vendor Account...</option>';
   
	$op = mysql_query("SELECT * FROM ledger_tabs WHERE saasid='".$_SESSION['saasid']."' AND active = 'true' AND is_vend = 'true'");
	while($ty = mysql_fetch_array($op)){
	echo '<option value="'.$ty["glid"].'">'.$ty["fld1"].'</option>';	
	}
...
<div style="width:120px; height:40px; float:left">
   GL Account:
    ...
    <div style="width:300px; height:40px; float:left">
    <select name="glacctpost" id="glacctpost" class="makerrsw">
    <option value="none">Select GL Account...</option>';
...
$glacctpost = mysql_real_escape_string($_REQUEST['glacctpost']);
$isvn = $_REQUEST['isvn'];
...
mysql_query("INSERT INTO transandpays SET date = '$billdate', numtype = 'Submitted Bill', gl_act = '$gl',	payment = '$amntdue', ref_id = '$refnum', vendor = '$ven', terms = '$terms', due_date = '$duedt', status = '$status', saasid = '".$_SESSION['saasid']."', gl_refatt = '$glacctpost'")or die(mysql_error());
...
mysql_query("UPDATE transandpays SET date = '$billdate', payment = '$amntdue', ref_id = '$refnum',  due_date = '$duedt', status = '$status' WHERE pyid = '".$_REQUEST['id']."'")or die(mysql_error());
...
$owedAmount =  number_format($totSet2 - $totSet3,2);
		...
		if($dud < .01){
			...
			$pb = mysql_fetch_array(mysql_query("SELECT * FROM ledger_tabs WHERE glid = '".$rgl["gl_act"]."' AND saasid = '".$_SESSION['saasid']."'"));
				if($pb["selterm"] != ''){
					
				$abc = mysql_fetch_array(mysql_query("SELECT * FROM ven_terms WHERE term_id = '".$pb["selterm"]."' AND saasid = '".$_SESSION['saasid']."'"));
					...
					$date = $rgl["date"];
					$newDate =  date('m/d/Y', strtotime($date."+$day days") );
					
					if(date('m/d/Y') < $newDate){
						
						$discount = $dud * ((100-$precent) / 100);
						
						//$discount = $dud - $precent;
						$outy = $dud - $discount; 
						
						$vtrim = number_format($dud - $outy,2);
						$vtrim2 = $dud - $outy;
						$afterDis = "$".number_format($dud - $outy,2);
						
						$outywha = format_currency($discount);
						
						//$outywha = number_format($outywha);
						if($vtrim < .01){
						$off = 'true';	
						mysql_query("UPDATE transandpays SET status = 'closed' WHERE pyid = '".$rgl["pyid"]."'");
						}else{
						...
						$disc = $bouncd2 += $vtrim2;
						$outy = "$".number_format($outy,2);
						
						$netty = "$precent% - $day days - Net ".$net;

						$goTot = $dud - $outywha;
				$theIcon = '<div style="margin-top:8px" class="info_icon" title="Discount of '.$outy.' will be applied if paid by: '.$newDate.'"></div>';
					}else{
					//$outy = 'Term Exceeded';
					$afterDis = "$".number_format($dud,2);
					$theIcon = '';
					}
...
<div class="headtext2" style="width: 120px;">'.$afterDis.'</div>
<div class="headtext2" style="width: 176px;">'.$netty.'</div>
...
<div class="delete_icon" title="Delete" onclick="delBill(\''.$rgl["pyid"].'\')"></div>
'.$theIcon.'
</div></div>';	
		}
			}
...
// print the navigation link
echo '<div style="clear:both; padding:10px; font-weight:bold; font-size:15px;"><div style="float:left">Totals:</div> <div style="float:left; margin-left:260px; width:85px;">$'.number_format($dud2,2).'</div> <div style="float:left; margin-left:26px; width:85px;">$'.number_format($dud2 - $trimsd,2).'</div></div>';
echo '<div style="padding-right:5px; padding-top:5px; float:right; margin-top:10px">'.$first .  $next .$nav . $last.'</div>
<div style="clear:both"></div>
';
}

if($act == 'deactivebills'){
	mysql_query("DELETE FROM transandpays WHERE pyid = '".$_REQUEST['bilid']."' AND saasid = '".$_SESSION['saasid']."'");
...
<div style="padding:10px; color:#33CC00;">If Amt to pay is green, discount is applied.</div>
...
<div class="headtext" style="width:100px; border-right:solid thin #000; border-left:solid thin #CCC;">Due Date</div>
<div class="headtext" style="width: 220px; border-right:solid thin #000; border-left:solid thin #CCC;">Vendor / Account</div>
<div class="headtext" style="width:130px; border-right:solid thin #000; border-left:solid thin #CCC;">Discount Amt</div>
<div class="headtext" style="width:110px; border-right:solid thin #000; border-left:solid thin #CCC;">Amt. Due</div>
<div class="headtext" style="width:110px; border-left:solid thin #CCC;">Amt. to Pay</div>
</div>
...
if($pb["selterm"] != ''){
				...
				$abc = mysql_fetch_array(mysql_query("SELECT * FROM ven_terms WHERE term_id = '".$pb["selterm"]."' AND saasid = '".$_SESSION['saasid']."'"));
					...
					if(date('m/d/Y') < $newDate){
						...
						$disc = $bouncd2 += $outy;
						//$outy = number_format($outy,2);
						$outyCol = 'color:#33CC00;';
						$disAmnt = format_currency($precent);
						
						$outy = $dud - $discount;
						$outy2 = "$".number_format($dud - $discount,2); 
					...
					$outy2 = number_format($dud,2);
					$outyCol = '';
					$disAmnt = number_format($dud,2);
					$vtrim = number_format($dud,2);
					}
...
<div class="headtext2" style="width:110px;">'.$rgl["due_date"].'</div>
<div class="headtext2" style="width:220px;">'.$name.'</div>
<div class="headtext2" style="width:140px;">'.$outy2.'</div>
<div class="headtext2" style="width:100px;">$'.str_replace(",","",$vtrim).'</div>
<div class="headtext2" style="width:120px;">$ <input class="billsty" style="width:95px; background-color:#fff; '.$outyCol.'" name="payamnt'.$rgl["pyid"].'" id="payamnt'.$rgl["pyid"].'" type="text" value="'.str_replace(",","",$vtrim).'"></div>
</div>';
...
$what = explode("=",$key);
	...
	if($totSet2 > $gh["fld4"]){
		 echo 'The selected bank account does not have enough funds to cover all payments applied.<br>Please select an alternate bank account to continue.';
	}else{
		foreach($tags as $key) {
$what = explode("=",$key);
...
$r=mysql_query("SELECT * FROM ledger_tabs WHERE active = 'true' AND saasid = '".$_SESSION['saasid']."' AND is_vend = 'true' OR active = 'true' AND saasid = '".$_SESSION['saasid']."' AND type = 'bank' ORDER BY fld1 ASC");
...
<div class="headtext2" style="width: 90px;"><input style="width:90px; background-color:#FFF;" name="amountded" id="amountded" type="text" onblur="runtoCred(this.value)"></div>
  ...
  <div class="infohead">
    <div class="headtext" style="width: 300px; border-right:solid thin #000; border-left:solid thin #CCC;">GL Account</div>
    <div class="headtext" style="width: 428px; border-right:solid thin #000; border-left:solid thin #CCC;">Memo</div>
...
echo '<div id="telem" style="margin-bottom:20px; font-size:17px">Post a Invoice Payment</div>
...
if($act == 'raddspays'){
	...
	$yo = mysql_fetch_array(mysql_query("SELECT * FROM core_docs WHERE doc_id = '".$rtog["inv_num"]."' AND saasid = '".$_SESSION['saasid']."'"));
		...
		if($amount > $deduct){
			...
			die('<strong style="color:#1053A3;">An error has occurred!</strong><br><span style="font-size:12px;">Amount submitted for payment is larger than invoice balance. Please re-enter.</span>');
...
///AUTO SEARCH FUNCTION FOR VENDORS////
if($act == 'possersvends'){
	if($_REQUEST['serval'] != 'all vendors'){
	$a = mysql_query("SELECT * FROM ledger_tabs WHERE fld1 LIKE '".$_REQUEST['serval']."%' AND is_vend = 'true' AND saasid = '".$_SESSION['saasid']."' AND active = 'true'")or die(mysql_error());
		while($b = mysql_fetch_array($a)){
			echo '<div style="padding:5px; cursor:pointer" onclick="setVendf(\''.$b["glid"].'\',\''.$b["fld1"].'\')">'.$b["fld1"].'</div>';
		}
	}else{
		$a = mysql_query("SELECT * FROM ledger_tabs WHERE is_vend = 'true' AND saasid = '".$_SESSION['saasid']."' AND active = 'true'")or die(mysql_error());
		while($b = mysql_fetch_array($a)){
			echo '<div style="padding:5px; cursor:pointer" onclick="setVendf(\''.$b["glid"].'\',\''.$b["fld1"].'\')">'.$b["fld1"].'</div>';
		}
	}
...
////GET ADDRESS///getaddress&glid
if($act == 'getaddress'){
	$a =mysql_fetch_array(mysql_query("SELECT * FROM ledger_tabs WHERE glid = '".$_REQUEST['glid']."' AND saasid = '".$_SESSION['saasid']."'"))or die(mysql_error());
	if($a["addressset"] != ''){
		echo ''.$a["addressset"].'<br>'.$a["fld7"].' '.$a["fld6"].','.$a["fld8"].'';
		
	}else{
	echo 'No Address in System';	
	}
}