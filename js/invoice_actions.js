$(function() {
		...
		$.ajax({
  url: 'inc/invoice_actions.php?action=getlist',
...
function recallEstis(){
  ...
  success: function(data) {
	...
	$.ajax({
  url: 'inc/invoice_actions.php?action=getstatdrop&setval='+val+'&sentid='+val2,
  success: function(data) {
	...
	function addProdiserv(){
		...
		if(er1 == '' && er2 == '' && er3 == ''){
		$.ajax({
  url: 'inc/invoice_actions.php?action=getproservlist',
  success: function(data) {
    $('#estiadd').html(data);
	...
	function getEstform(){
		$.ajax({
  url: 'inc/invoice_actions.php?action=getaddest',
  success: function(data) {
    $('#esti_content').html(data);
	...
	function runCons(val){
		...
		$.ajax({
  url: 'inc/invoice_actions.php?action=getcons&compid='+val,
  success: function(data) {
    $('#estconssd').html(data);
	...
	function runLocs(val){
		//alert(val);
		$.ajax({
  url: 'inc/invoice_actions.php?action=getlocs&compid='+val,
  success: function(data) {
    $('#estlocss').html(data);
	...
	function getTaxs(val){
		...
		$.ajax({
  url: 'inc/invoice_actions.php?action=gettaxs&compid='+val,
  success: function(data) {
    $('#taxcont').html(data);
	...
	var productservi = $("#productservi").val();
		...
		$.ajax({
  url: 'inc/invoice_actions.php?action=additem&itemid='+producuid+'&quant='+quant+'&docid='+docid+'&istax='+tax+'&proservprice='+proservprice,
  success: function(data) {
  $("#estiadd").dialog( "close" );
	...
	function getItemlist(){
		...
		var estloc = $("#estloc").val();
		$.ajax({
  url: 'inc/invoice_actions.php?action=getitmelist&doc_id='+doc_id+'&locid='+estloc,
  success: function(data) {
   $("#itmhold").html(data);
	}
  });	
	...
	function goTotal(){
		...
		var estloc = $("#estloc").val();
	$.ajax({
  url: 'inc/invoice_actions.php?action=grbmoney&docid='+doc_id+'&locid='+estloc,
  success: function(data) {
  $("#tothold").html(data);
  }
  });		
	...
	var comp = $("#comp").val();
		...
		if(er1 == '' && er2 == '' && er3 == ''){
			...
			$.ajax({
  url: 'inc/invoice_actions.php?action=insertestimate&docid='+doc_id+'&comp='+comp+'&estcont='+estcont+'&RadioGroup1='+RadioGroup1+'&salesman='+salesman+'&payterms='+payterms+'&notes='+notes+'&estiamount='+estiamount+'&status='+status+'&locid='+estloc+'&estdt='+estdt+'&poapp='+poapp,
  success: function(data) {
  recallEstis();
  $("#confrmsucc").fadeIn('slow');
	...
	function geteditEstform(val){
		$.ajax({
  url: 'inc/invoice_actions.php?action=editest&docid='+val,
  success: function(data) {
    $('#esti_content').html(data);
	 ...
	 icons: {
		...
		var spl = accesselement.split(',');
			...
			if(writedoc == 'true'){
				...
				$(".addnew, #addcrt, #addsestbut").removeAttr('onclick');
	...
	var comp = $("#comp").val();
		...
		if(er1 == '' && er2 == '' && er3 == ''){
			...
			$.ajax({
  url: 'inc/invoice_actions.php?action=editestimate&docid='+doc_id+'&estcont='+estcont+'&RadioGroup1='+RadioGroup1+'&salesman='+salesman+'&payterms='+payterms+'&notes='+notes+'&estiamount='+estiamount+'&status='+status+'&locid='+estloc+'&estdt='+estdt+'&poapp='+poapp,
  success: function(data) {
  recallEstis();
  $("#confrmsucc").fadeIn('slow');
		 ...
		 $("#alerts").html('<strong style="color:#1053A3;">Are you sure you wish to delete this item?</strong><br>');
			...
			buttons: {
				...
				'Yes': function() {
					...
					$.ajax({
  url: 'inc/invoice_actions.php?action=delitem&itmid='+val,
  success: function(data) {
 getItemlist();
 $("#confrmsucc").fadeIn('slow');
	...
	function upqut(val){
		$.ajax({
  url: 'inc/invoice_actions.php?action=pullqtfld&itmid='+val,
  success: function(data) {
 ...
 $("#updtqt"+val).focus();
	...
	function editItm(val){
		...
		$.ajax({
  url: 'inc/invoice_actions.php?action=updtqty&itmid='+val+'&newvalue='+newqut,
  success: function(data) {
getItemlist();
$("#confrmsucc").fadeIn('slow');
			...
			buttons: {
				...
				'Yes': function() {
					$( this ).dialog( "close" );
						...
						$.ajax({
  url: 'inc/invoice_actions.php?action=delesti&docid='+val,
  success: function(data) {
recallEstis();
$("#confrmsucc").fadeIn('slow');
			...
			buttons: {
				...
				'Yes': function() {
					$( this ).dialog( "close" );
						...
						$.ajax({
  url: 'inc/invoice_actions.php?action=changelinestat&docid='+val+'&statu='+val2,
  success: function(data) {
recallEstis();
$("#confrmsucc").fadeIn('slow');
...
$(function() {
   ...
   $.ajax({
  url: 'inc/invoice_actions.php?action=ramitems',
  success: function(data) {
  }
  });
...
function makePay(val){
	
	 $.ajax({
  url: 'inc/invoice_actions.php?action=grabform&id='+val,
  success: function(data) {
	$( "#dio1" ).html(data); 
	$("#telem").html('Customer Payment for Invoice: '+val);
$( "#dio1" ).dialog({
			width: 650,
			resizable: false,
			modal: true,
			title: "Post a Customer Payment",
		});	
...
if(er1 == '' && er2 == '' && er3 == '' && er4 == ''){
 $.ajax({
  url: 'inc/invoice_actions.php?action=raddspays&setdate='+setdate+'&checknum='+checknum+'&amount='+amount+'&notes='+notes+'&bank='+bankset+'&invid='+val,
  success: function(data) {
	  if(data == 'good'){
		  recallEstis();
	  $( "#dio1" ).dialog( "close" );
